...
<div class='row-fluid'>
	<div class='span12'>
		<div class='page-header'>
			...
			<span><fmt:message key="joblist.info.job" /><!--Jobs--></span>
			</div>
			
			<div class="pull-right"> 	
	            	<button id="inplaceediting-enable" class="btn btn-success" onclick="job.create();"><i class="icon-plus "></i> <span><fmt:message key="global.info.new" /><!--New--></span></button>
					<button id="inplaceediting-enable" class="btn btn-danger" onclick="job.remove();"><i class="icon-trash"></i> <span><fmt:message key="global.info.delete" /><!--Delete--></span></button>
			</div>
		</div>
	</div>
</div>
			...
			<div class="box-content box-no-padding">
				<div class="responsive-table">
					<div class="scrollable-area">
						<table id="job-data-table" style="margin-bottom: 0;"
							...
							<c:forEach var="job" items="${jobs}">
								<tr>
									...
									<td>${job.name}&nbsp;</td>
									<td>${job.punchCode}&nbsp;
									<c:if test="${job.punchCode==''}">-</c:if>
									</td>		
									 <td><c:if test="${job.active}"><i class="icon-check"></i></c:if><c:if test="${!job.active}"><i class="icon-check-empty "></i></c:if> <fmt:message key="global.info.active" /></td>								
									<td>
										<div class="text-left">
											<a href="javascript:;" onclick="job.update('${job.id}')" class="btn btn-warning btn-mini"> 
