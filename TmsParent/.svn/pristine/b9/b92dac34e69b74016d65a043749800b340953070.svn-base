    ...
    $(document).ready(function() {
        ...
        $("#main-nav .dropdown-collapse").on("click", function(e) {
            ...
            if (list.is(":visible")) {
            	...
            	if($(this).attr("class").indexOf("leaf-node")==-1){
            		$("#navigation_ul ul").slideUp(300, function () {
        ...
        nav_toggler.on("click", function() {
            if (nav_open) {
                ...
                $("nav .toggle-nav i").attr("class","icon-list");
                $("nav .toggle-nav i").attr("title","显示菜单");
            } else {
                $(document).trigger("nav-open");
                $("nav .toggle-nav i").attr("class","icon-align-justify");
                $("nav .toggle-nav i").attr("title","收起菜单");
            }
            return false;
        });
