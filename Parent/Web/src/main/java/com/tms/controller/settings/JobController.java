...
public class JobController extends BaseController<Object> {
	...
	@RequestMapping("/list")
	public ModelAndView jobList(HttpServletResponse response,
	...
	public ModelAndView jobListValue(HttpServletResponse response,
			HttpServletRequest request) throws Exception {
	...
	public ModelAndView createJob(HttpServletResponse response,
			HttpServletRequest request) throws Exception {
		ModelAndView view = new ModelAndView();
		User user = SessionUtils.getUser(request);
		List<Task> taskList = tmsService
				.findAllTasks(user.getCompany().getId());
	...
	public ModelAndView updateJob(String id, HttpServletResponse response,
			HttpServletRequest request) throws Exception {
		ModelAndView view = new ModelAndView();
	...
	@RequestMapping("/save")
	public ModelAndView saveJob(@RequestBody JobVo jobVo,
		...
		try {
			...
			if (StringUtils.isEmpty(job.getId())) {
				if (tmsService.validateJobCodeNamePunchCodeExist(job.getCode(),
						job.getName(), job.getPunchCode(), user.getCompany()
								.getId())) {
				...
				if (tmp.getCode().equals(job.getCode())
						&& tmp.getName().equals(job.getName())
						&& tmp.getPunchCode().equals(job.getPunchCode())) {
					...
					if (tmsService.validateJobCodeNamePunchCodeExist(code,
							name, punchCode, user.getCompany().getId())
							&& punchCode != "") {
						view.getModel().put("message", "exist");
						return view;
					} else {
	...
	public ModelAndView deleteJob(String ids, HttpServletResponse response,
			HttpServletRequest request) throws Exception {
		ModelAndView view = new ModelAndView();
		String[] jobIds = ProcessSignUtils.getInstance().processComma(ids);
