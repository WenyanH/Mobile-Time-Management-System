...
public class PageViewTag extends TagSupport {
    ...
    private static final String HREF_START = "<li><a href='";
    private static final String HREF_AJAX_START = "<li><a  href='javascript:void(0)' onclick='retrieveURL(\"";

    private static final String PAGE_DIV_START = "<div class=\"{0}\" style=\"{1}\"><ul class=\"pager\">";

    private static final String PAGE_DIV_END = "</ul></div>";

    private static final String HREF_END = "</a></li>" + SPACE;
        ...
        if (totalPages > 1) {
            ...
            if (currentPage > 1) {
                sb.append(HREF_AJAX_START + action + "?page=" + (currentPage - 1) + "&count=" + pageCount
                        + paraBuffer.toString().replaceAll("&pjax=true", "") + "\",\"" + render + "\"," + callbackBuffer.toString() + ")'>«"
                        + HREF_END);
            } else {
            ...
            if (totalPages > 8) {
                ...
                if (currentPage < 8) {
                    ...
                    sb.append("<li class=\"separator\"><span>...&nbsp;</span></li>");
                    for (int i = currentPage - 3; i < currentPage; i++) {
                        sb.append(HREF_AJAX_START + action + "?page=" + i + "&count=" + pageCount
                                + paraBuffer.toString().replaceAll("&pjax=true", "") + "\",\"" + render + "\"," + callbackBuffer.toString() + ")'>"
                                + i + HREF_END);
                    }
                    sb.append("<li class=\"active\"><a>"+currentPage + "&nbsp;</a></li>");
                ...
                for (int i = 1; i <= totalPages; i++) {
                    String url = null;
                    if (i == currentPage) {
                        url = "<li class=\"active\"><a>" + i + "</a></li>";
                    } else {
                        url = HREF_AJAX_START + action + "?page=" + i + "&count=" + pageCount + paraBuffer.toString().replaceAll("&pjax=true", "")
        ...
        if (totalPages > 1) {
            ...
            if (currentPage > 1) {
                sb.append(HREF_START + action + "?page=" + (currentPage - 1) + "&count=" + pageCount
                        + paraBuffer.toString().replaceAll("&pjax=true", "") + "'>«" + HREF_END);
            } else {
                sb.append("<li class=\"disabled\"><a>«</a></li>");
            }
            ...
            if (totalPages > 8) {
                    ...
                    for (int i = 1; i <= 8; i++) {
                        if (i == currentPage) {
                            ...
                            sb.append(HREF_START + action + "?page=" + i + "&count=" + pageCount
                                    + paraBuffer.toString().replaceAll("&pjax=true", "") + "'>" + i + HREF_END);
                        }
                    }
                } else {
                    for (int i = 1; i <= 2; i++) {
                        sb.append(HREF_START + action + "?page=" + i + "&count=" + pageCount + paraBuffer.toString().replaceAll("&pjax=true", "")
                                + "'>" + i + HREF_END);
                    }
                    sb.append("<li class=\"separator\"><span>...&nbsp;</span></li>");
                    for (int i = currentPage - 3; i < currentPage; i++) {
                ...
                for (int i = 1; i <= totalPages; i++) {
                    ...
                    if (i == currentPage) {
                        ...
                        url = HREF_START + action + "?page=" + i + "&count=" + pageCount + paraBuffer.toString().replaceAll("&pjax=true", "") + "'>"
                                + i + HREF_END;
                    }
                    sb.append(url);
                }
