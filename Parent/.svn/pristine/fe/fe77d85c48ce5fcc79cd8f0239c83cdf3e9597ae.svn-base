...
<head>
	...
	<link href="<webpath:path/>/resources/default/css/schedule.css" media="all" rel="stylesheet" type="text/css" />
	<script src="<webpath:path/>/resources/default/js/tms/tmssetting/schedule.js" type="text/javascript"></script>
	<script src="<webpath:path/>/resources/default/js/schedule.js" type="text/javascript"></script>
</head>
<div class='row-fluid'>
	<div class='span12'>
	...
	<input type="hidden" id="rowCount">
    		<input type="hidden" id="beginHour">
    		<input type="hidden" id="beginMinute">
    		<input type="hidden" id="startDay" value="0">
	...
	<div class='row-fluid' >
		<div class='span4' >
			...
			<div class="control-group">
				...
				<div class="controls">
					<c:if test="${schedule.id!=null}"><input class="span12" name="period" type="number" min="1" id="period" value="${schedule.period}" readonly/></c:if>
					<c:if test="${schedule.id==null}"><input class="span12" name="period" type="number"  min="1" id="period" value="1" readonly/></c:if>
				</div>
			</div>	
			...
			<div class="control-group">
				...
				<div class="controls">
					<c:if test="${schedule.id!=null}"><input class="span12" name="days" type="number" id="days" value="${schedule.days}" readonly/></c:if>
					<c:if test="${schedule.id==null}"><input class="span12" name="days" type="number" id="days" value="7" readonly/></c:if>
...
<script>
	$(document).ready(function(){
		...
		function getValue(){
	    	...
	    	var beginHour = parseInt($("#beginHour").val());
	    	var beginMinute = parseInt($("#beginMinute").val());

	    	var startDay = parseInt($("#startDay").val());
		...
		$('#calendar').fullCalendar({
			...
			eventClick: function(calEvent, jsEvent, view) {
				//schedule.appendscheduletime('${schedule.id}',calEvent.id);
    		},
			//eventClick: function(calEvent, jsEvent, view) {
